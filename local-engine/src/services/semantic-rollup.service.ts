...
function addDirectEntityMemberships(
  ...
  for (const entity of entities) {
    ...
    addMembership(memberships, graphNodeId, entity.id, 1, graphNodeId);
  }
}
...
function addPathTokenMemberships(
  ...
  for (const node of graph.nodes) {
    ...
    if (!semanticEntityId) continue;
    addMembership(memberships, node.id, semanticEntityId, 0.7, node.id);
  }
}
...
export function buildSemanticRollup(
  ...
  return {
    memberships,
    relations: aggregateRelations(graph, entities, memberships),
  };
}
