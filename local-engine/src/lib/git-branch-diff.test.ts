...
import { readBranchDiff } from "./git-branch-diff.js";
...
describe("readBranchDiff", () => {
  it("returns empty diff when base equals head", async () => {
    const runGit = mockRunGit({
      "for-each-ref --format=%(refname:short) refs/heads": "main\0feature-a\0",
    });
  ...
  it("rejects unknown branches", async () => {
    const runGit = mockRunGit({
      "for-each-ref --format=%(refname:short) refs/heads": "main\0",
    });
